    ...
    if (args.userName.length < 3 || args.userName.length > 20) {
      return {
        errors: [
          {
            field: 'userName',
            message: 'User Name must be between 3 and 20 characters',
          },
        ],
        ...
        errors: [
          {
            field: 'password',
            message:
              'Invalid Password. Should contain 8-20 characters, 1 Upper Case, 1 Lower Case, 1 digit, 1 Special Character',
          },
        ],
        code: this.responseCode.InvalidInput,
      };
