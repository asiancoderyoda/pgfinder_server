...
class AuthService extends Repository<UserEntity> {
  ...
  public async authenticate(userName: string, password: string) {
    ...
    const token = jwt.sign(jwtUser, process.env.JWT_KEY || 'jwt_secret', {
      expiresIn: this.AUTH_EXPIRES_IN,
    });
    const refreshToken = jwt.sign(
      jwtUser,
      process.env.JWT_REFRESH_KEY || 'jwt_secret',
  ...
  public verifyUser(
    bearerTokenCombi: string,
    callback: (user: null | undefined | jwt.JwtPayload) => void
  ) {
    ...
    jwt.verify(token, process.env.JWT_KEY || 'jwt_secret', (err, decoded) => {
      if (err) {
        this.errLogger.error(
          'Service: auth.middleware - verify user - Error: ' + err
        );
        return callback(null);
      }
